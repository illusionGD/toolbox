...
- 只重命名文件，不重命名文件夹。
- 添加文件夹上限 5 万（重命名是逐个 syscall，量级比只读统计小）。导入本身走 `useFolderImport`（见 [[common-capabilities]]，四页共用），**本页不传 `accept`**——重命名对文件类型没有偏好，过滤反而挡住用户想改的文件。
- 「包含子文件夹」是**工具栏复选框**，不在参数面板里。它影响的是「下一次点添加文件夹」这个动作，跟规则链那些「改一下整表立刻重算」的参数不是一类东西，放在一起会让人以为改完当前列表会跟着变。图片三页同位置同写法。
- **本页没有分页**（图片三页有）。分页切片按 `items` 顺序算，而这里的表格是**受控列排序**（`{n}` 序号必须跟着表格显示顺序走），两者叠加要先排序再切片、且序号得按全局位置而非页内位置算。列表没有缩略图那种「每行一次主进程调用」的开销，分页换不来什么，暂不做。
