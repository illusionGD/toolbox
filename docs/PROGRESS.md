...
- **当前任务**：#19 续「视频剪切 / 画面裁剪」🔵 **待 review** —— 把 #19 已在主进程实现却没接 UI 的 `trim`+`crop` 接成 `/media/video` 的第二个 tab（`VideoCompressView.vue` → `VideoView.vue`），同时修了三处只有接 UI 才会暴露的缺陷。验证 39 条断言全绿、typecheck/lint/format/build 全绿。
- **下一步**：待用户定。候选：P5 #21 host 工具（唯一还没动的 PLAN 项，要处理管理员权限）/ 补 #15 遗留的「每个 sheet 各出一套 JSON」/ 视频分段导出（本轮只做单段）。
- **待人工验**：① #18 的图集观感只有人能判断 —— 生成后把 `.fnt` + PNG 丢进 Pixi 或在线 BMFont 查看器确认渲染正确；② 本轮剪切页 —— 拖选区导出后播放接缝是否连贯、裁剪框是否所见即所得、`copy` 模式起点偏移与文案是否一致。
